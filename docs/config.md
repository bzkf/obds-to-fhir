...
You can enable a dead letter queue to receive all records where an exception was thrown during processing by adding
the `dlq` profile to the list of active Spring profiles when running the application.
Simply set the environment variable: `SPRING_PROFILES_ACTIVE=mappings,default,dlq`.

Note that `SPRING_PROFILES_ACTIVE` should always contain the `mappings` profile.

Both records processed by the oBDS processor and the one mapping the extra patient data (BestOfTumor) are sent to the same
error topic.

> [!IMPORTANT]
> The FHIR bundles are generated by aggregating all reports per patient and tumor.
> Enabling the DLQ causes individual reports to be ignored during processing, potentially
> causing inconsistent or implausible FHIR patient records. We recommend to only use the
> DLQ to assess the extent of faulty reports and suggest fixing all of them or raising
> an issue on GitHub so we can possibly make the mapping more lenient.
...
| fhir.mappings.create-patient-resources.use-obds-patient-id-as-resource-id | bool | `false` | If enabled, sets Patient.id to the mapped Patient_ID value instead of a hash of the identifier. |
...
| fhir.mappings.patient-reference-generation.fhir-server.strip-legacy-media-types-from-accept-header | bool | `false` | some FHIR servers reject requests whose `Accept` header contains the long-deprecated `application/xml+fhir`/`application/json+fhir` media types, which HAPI's FHIR client always includes as a low-priority fallback alongside the current `application/fhir+xml`/ `application/fhir+json` types. Enable this to strip the deprecated media types from the `Accept` header sent to `fhir-server.base-url`. |
...
| spring.kafka.producer.properties."[linger.ms]" | string | `"${KAFKA_PRODUCER_LINGER_MS:200}"` | Accumulate records during the replication round-trip; free throughput gain with acks=all. |
| spring.kafka.producer.properties."[batch.size]" | string | `"${KAFKA_PRODUCER_BATCH_SIZE:1048576}"` | 1 MB batch size; improves zstd compression ratio for large FHIR bundles. |
| spring.kafka.producer.properties."[buffer.memory]" | string | `"${KAFKA_PRODUCER_BUFFER_MEMORY:67108864}"` |  |
| spring.kafka.producer.properties."[request.timeout.ms]" | string | `"${KAFKA_PRODUCER_REQUEST_TIMEOUT_MS:60000}"` | Must exceed replication RTT on slow inter-broker network with min.isr=3. |
| spring.kafka.producer.properties."[delivery.timeout.ms]" | string | `"${KAFKA_PRODUCER_DELIVERY_TIMEOUT_MS:180000}"` | Must be > request.timeout.ms to leave room for retries. |
