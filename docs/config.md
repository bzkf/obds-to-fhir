...
| fhir.mappings.create-patient-resources.if-already-exists | bool | `false` | If set to true, a Patient resource is always created and added to the bundle, even if the patient already exists on the FHIR server. The patient reference for all other resources in the bundle is overridden to refer to Patient.id (the locally-computed hash-based ID), while the logical identifier reference is preserved. |
