...
public class OperationMapper extends ObdsToFhirMapper {
  ...
  public Procedure map(OPTyp op, Reference subject, Reference condition) {
    ...
    // there could be multiple OPS codes here --> create one coding for each, iterate through ops codes+
    ...
    for (var opsCode : op.getMengeOPS().getOPS()) {
      ...
      Coding ops =
        new Coding(fhirProperties.getSystems().getOps(), opsCode.getCode(), "");
      ...
      procedure.addExtension(
          fhirProperties.getExtensions().getMiiExOnkoOPIntention(), intention);
